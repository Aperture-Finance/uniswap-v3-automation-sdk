...
describe('Viem - Routing tests', function () {
  ...
  it('Test optimalRebalanceV2 in arbitrum', async function () {
    ...
    for (let i = 0; i < resultV2.length; i++) {
      ...
      expect(Number(resultV2[i].feeUSD)).to.be.closeTo(0.2055, 0.002); // swap ~3.8 USDC, reinvest ~$1.62, and FEE_REBALANCE_USD, totalFeeUsd=0.2055
      expect(Number(resultV2[i].feeBips) / 1e18).to.be.closeTo(0.0232, 0.005); // position $8.87, bips 0.2055/8.87 = ~0.0232
