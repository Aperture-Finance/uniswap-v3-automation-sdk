...
describe('Viem - Routing tests', function () {
  ...
  it('Test optimalRebalanceV2 in arbitrum', async function () {
    ...
    for (let i = 0; i < resultV2.length; i++) {
      ...
      expect(Number(resultV2[i].feeUSD)).to.be.closeTo(0.1526, 0.0003); // swap ~3.8 USDC, fee 0.1526
      expect(Number(resultV2[i].feeBips) / 1e18).to.be.closeTo(0.0172, 0.005); // position $8.87, bips 0.1526/8.87 = ~0.0172
