...
async function main() {
  ...
  for (const swapInfo of swapInfos) {
    ...
    console.log(
      `solver=${solver}, liquidity: ${swapInfo.liquidity}, amount0=${amount0}, amount0=${amount1}, deadline=${deadline}, tokenInFeeAmount=${(token0FeeAmount ?? 0n) + (token1FeeAmount ?? 0n)}, swapData=${swapData}, swapData1=${swapData1}, txRequest=${JSON.stringify(txRequest)}`,
    );
  }
  process.exit(0);
}
