...
describe('normalizeTicks', () => {
  ...
  it('should return the nearest usable normalized tick range for RecurringPriceDual', async () => {
    // Setup.
    const amm = AutomatedMarketMakerEnum.enum.UNISWAP_V3;
    const chainId = ApertureSupportedChainId.ETHEREUM_MAINNET_CHAIN_ID;
    const lteOffset = 50;
    const gteOffset = 100;
    const action = {
      type: ActionTypeEnum.enum.RecurringPercentageDual,
      slippage: 0.05,
      maxGasProportion: 0.1,
      lteTickLowerOffset: -lteOffset,
      lteTickUpperOffset: lteOffset,
      gteTickLowerOffset: -gteOffset,
      gteTickUpperOffset: gteOffset,
    };
    const [tokenA, tokenB] = await Promise.all([
      getToken(ETHEREUM_MAINNET_ETH, chainId),
      getToken(ETHEREUM_MAINNET_USDC, chainId),
